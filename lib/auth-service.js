...
const USER_REGISTRY_KEY = process.env.AUTH_USER_REGISTRY_KEY || "dz:auth:users:v1";
const MAX_USER_REGISTRY_ITEMS = Math.max(parseInt(process.env.AUTH_MAX_USERS, 10) || 20000, 100);
...
async function listUserIds() {
  const loaded = await getStore().getJSON(USER_REGISTRY_KEY);
  if (!Array.isArray(loaded)) return [];
  return loaded
    .map((entry) => String(entry || "").trim())
    .filter(Boolean)
    .slice(-MAX_USER_REGISTRY_ITEMS);
}

async function registerUserId(userId) {
  const normalizedUserId = String(userId || "").trim();
  if (!normalizedUserId) return;
  const listed = await listUserIds();
  if (listed.indexOf(normalizedUserId) !== -1) return;
  listed.push(normalizedUserId);
  if (listed.length > MAX_USER_REGISTRY_ITEMS) {
    listed.splice(0, listed.length - MAX_USER_REGISTRY_ITEMS);
  }
  await getStore().setJSON(USER_REGISTRY_KEY, listed);
}
...
async function saveUser(user) {
  ...
  await registerUserId(normalized.id);
  return normalized;
}

function matchesUserQuery(user, rawQuery) {
...
async function listUsers(options) {
  ...
  const roleFilter = String(opts.role || "").trim().toLowerCase();
  const role = roleFilter === "admin" || roleFilter === "user" ? roleFilter : "";
  const query = String(opts.query || opts.q || "").trim();

  const ids = (await listUserIds()).slice().reverse();
  const matched = [];
  for (let i = 0; i < ids.length; i += 1) {
    const user = await getUserByID(ids[i]);
    if (!user) continue;
    if (role && user.role !== role) continue;
    if (!matchesUserQuery(user, query)) continue;
    matched.push(toPublicUser(user));
  }

  const items = matched.slice(cursor, cursor + limit);
  const nextCursor = cursor + limit < matched.length ? String(cursor + limit) : null;
  return {
    items: items,
...
async function loginUser(email, password, req) {
  ...
  const computedRole = computeRoleByEmail(user.email);
