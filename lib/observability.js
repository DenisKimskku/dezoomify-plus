...
function recordMetric(key, nowTs) {
...
function recordHttpRequest(endpoint, statusCode, durationMs, extra) {
  ...
  recordMetric("http.requests.total", now);
  recordMetric("http.requests.endpoint." + ep, now);
  recordMetric("http.status." + ep + "." + statusClass, now);
  recordMetric("http.duration." + ep + "." + latencyClass, now);
  const reason = sanitizeMetricLabel(extra && extra.reason, "");
  if (reason) {
    recordMetric("http.reason." + ep + "." + reason, now);
  }
  const quotaType = sanitizeMetricLabel(extra && extra.quotaType, "");
  if (quotaType) {
    recordMetric("http.quota_type." + ep + "." + quotaType, now);
  }
}
